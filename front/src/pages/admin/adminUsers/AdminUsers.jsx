...
const AdminUsers = () => {
  ...
  const getAllData = async () => {
    const res = await axios.get(
      `http://localhost:5000api/users/${user.user._id}`
    );
    setData(res.data);
  };
  ...
  const makeAdmin = async (e, id, row, role) => {
    ...
    try {
      const response = await axios.put(
        `http://localhost:5000api/users/role/${id}`,
        dataToUpdate,
        {
          headers: {
    ...
    try {
      ...
      if (row.isActive) {
        response = await axios.delete(
          `http://localhost:5000api/users/${row._id}`,
          {
            headers: {
              Authorization: `Bearer ${user?.user?.token}`,
            },
        ...
        response = await axios.post(
          `http://localhost:5000api/users/restore`,
          {
            email: row?.email,
          },
          {
