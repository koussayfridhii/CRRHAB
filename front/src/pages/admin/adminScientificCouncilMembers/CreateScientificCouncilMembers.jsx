...
const CreateScientificCouncilMembers = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (path === "") {
        await axios.post(
          "https://crrhab.agrinet.tn/api/scientific_council",
        ...
        await axios.put(
          `https://crrhab.agrinet.tn/api/scientific_council/${formData._id}`,
          JSON.stringify(payload),
          {
            headers: {
              "Content-Type": "application/json",
  ...
  const firstApiCall = async (path) => {
    try {
      const response = await axios.get(
        `https://crrhab.agrinet.tn/api/scientific_council/${path}`
      );
